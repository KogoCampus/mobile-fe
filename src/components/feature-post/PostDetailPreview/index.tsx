...
const PostDetailPreview: React.FC<PostPreviewProps> = ({
  ...
  const [placeholderImages, setPlaceholderImages] = useState(
    new Array(imagesUrl.length).fill(false)
  );
  ...
  return (
    <View className="bg-white rounded-lg w-full">
      <Typography className="text-sm text-gray-500 mb-1 px-4">
        {topicId}
      </Typography>


      <Typography className="text-lg font-bold mb-1 text-black px-4">
        {title}
      </Typography>


      {imagesUrl.length > 0 && (
        <View className="flex-row mb-2 px-4">{renderImages()}</View>
      )}


      <Typography className="text-sm text-gray-600 mb-2 px-4">
        {content}
      </Typography>
