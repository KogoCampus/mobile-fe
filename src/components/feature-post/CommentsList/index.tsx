...
const CommentItem: React.FC<{
    ...
    return (
        <View className="bg-white p-4 mb-2 rounded-lg flex-row justify-between items-start">
            <View style={{ flex: 1 }}>
                <View className="flex-row items-center">
                    <View className="flex-1">
                        ...
                        <Typography className="text-gray-500 text-xs">{formatDate(comment.createdAt)}</Typography>
                    </View>
                </View>
                <Typography className="mt-2 mb-2 text-gray-700">{comment.content}</Typography>
            ...
            <TouchableOpacity onPress={toggleLike} className="flex-row items-center ml-2 mt-1">
                <AntDesign name={liked ? 'heart' : 'hearto'} size={16} color="#B10606" />
...
const RepliesSection: React.FC<{
    ...
    const { data: repliesData, isLoading, error } = useGetRepliesByCommentID(topicID, postID, parentComment.id);
            ...
            <ScrollableList intent="vertical" style={{ padding: 8, flex: 1 }}>
                ...
                <View className="ml-4">
                    {isLoading ? (
                        ...
                        repliesData?.data?.map(reply => <CommentItem key={reply.id} comment={reply} />)
                    )}
                </View>
            </ScrollableList>
            <View className="flex-row items-center border-t border-gray-200 p-4 bg-white">
                <TextInput className="flex-1 h-10 px-4 bg-gray-100 rounded-full" placeholder="Add a reply..." />
                <TouchableOpacity>
...
const CommentsSection: React.FC<{ topicID: string; postID: string }> = ({ topicID, postID }) => {
    const [showingReplies, setShowingReplies] = useState(false);
    const [parentComment, setParentComment] = useState<Comment | null>(null);

    const { data: commentsData, isLoading, error } = useGetCommentsByPostID(topicID, postID);
    ...
    const renderCommentItem = (comment: Comment) => {
        return <CommentItem key={comment.id} comment={comment} onReplyPress={handleReplyPress} />;
    };
    ...
    return (
        <KeyboardAvoidingView
            ...
            <View className="flex-1">
                ...
                <View className="flex-row items-center border-t border-gray-200 p-4 bg-white">
                    <TextInput className="flex-1 h-10 px-4 bg-gray-100 rounded-full" placeholder="Add a comment..." />
                    <TouchableOpacity>
                        <Ionicons name="send" size={24} color="blue" />
                    </TouchableOpacity>
                </View>
